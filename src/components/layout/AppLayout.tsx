...
import { Home, ListPlus, Clock, User, LogOut, Settings } from 'lucide-react';
  ...
  return (
    <div className="h-[100dvh] bg-gray-50 dark:bg-gray-900 flex flex-col md:flex-row w-full max-w-7xl mx-auto shadow-2xl relative transition-colors duration-200 overflow-hidden">
        ...
        <nav className="flex-1 p-4 space-y-2">
          {navItems.map((item) => (
            <button
              ...
              className="flex items-center space-x-3 w-full px-4 py-3 rounded-xl text-gray-600 dark:text-gray-300 hover:bg-green-50 dark:hover:bg-gray-700/50 hover:text-green-600 dark:hover:text-green-400 font-semibold transition-all min-h-touch"
            >
              {item.icon}
      ...
      <header className="md:hidden bg-white dark:bg-gray-800 px-4 py-3 flex items-center justify-between border-b dark:border-gray-700 z-10 shrink-0">
        ...
        <button onClick={handleSignOut} className="text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors p-2 min-h-touch min-w-touch flex items-center justify-center">
          <LogOut className="w-5 h-5" />
        </button>
      </header>
      ...
      <nav className="md:hidden bg-white dark:bg-gray-800 border-t dark:border-gray-700 flex justify-around items-center py-2 px-2 w-full z-10 shrink-0 safe-area-pb transition-colors duration-200">
        {navItems.map((item) => (
          <button
            key={item.label}
            onClick={() => navigate(item.path)}
            className="flex flex-col items-center justify-center w-16 h-12 rounded-lg text-gray-500 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-500 hover:bg-green-50 dark:hover:bg-gray-700 transition-all focus:outline-none min-h-touch"
          >
            {item.icon}
